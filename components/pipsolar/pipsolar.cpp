  ...
  if (this->state_ == STATE_POLL_DECODED) {
    ...
    switch (this->used_polling_commands_[this->last_polling_command_].identifier) {
      ...
      case POLLING_QFLAG:
        ...
        // additional switches add TiiMsvk 2024
		// x backlight
        if (this->backlight_switch_) {
          this->backlight_switch_->publish_state(value_backlight_on_ == 1);
        }
		// a buzzer
        if (this->buzzer_switch_) {
          this->buzzer_switch_->publish_state(value_silence_buzzer_open_buzzer_ == 1);
        }
		// b overload bypass function
        if (this->overload_bypass_switch_) {
          this->overload_bypass_switch_->publish_state(value_overload_bypass_function_ == 1);
        }
		// k lcd escape to default
        if (this->lcd_default_switch_) {
          this->lcd_default_switch_->publish_state(value_lcd_escape_to_default_ == 1);
        }
		// u overload restart function
        if (this->overload_restart_switch_) {
          this->overload_restart_switch_->publish_state(value_overload_restart_function_ == 1);
        }
		// v over temperature
        if (this->over_temperature_switch_) {
          this->over_temperature_switch_->publish_state(value_over_temperature_restart_function_ == 1);
        }
		// y alarm
        if (this->alarm_primary_interrupt_switch_) {
          this->alarm_primary_interrupt_switch_->publish_state(value_alarm_on_when_primary_source_interrupt_ == 1);
        }
		// z fault code record
        if (this->fault_record_switch_) {
          this->fault_record_switch_->publish_state(value_fault_code_record_ == 1);
        }
		// j power saving
